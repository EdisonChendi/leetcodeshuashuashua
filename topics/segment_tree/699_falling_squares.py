...
class Solution:
    def fallingSquares(self, positions: List[List[int]]) -> List[int]:
        ...
        def compression(positions):
            s = set()
            for p,l in positions:
                s.add(p)
                s.add(p+l)
            return {n:i for i,n in enumerate(sorted(s))}

        pos = compression(positions)
        res = []
        L = 2**(math.ceil(math.log(len(pos))/math.log(2))+1)
        segtree = SegmentTree(0, L)
        for p,l in positions:
            s, e, h =p,p+l, l
            si, ei = pos[s], pos[e]
            cur_h = segtree.query(si, ei-1)
            segtree.update(si, ei-1, h+cur_h)
            res.append(segtree.query(0, L))
        return res
